...
WHA_FIRS = {
    'MMEX': {'country': 'Mexico',    'flag': '🇲🇽', 'name': 'Mexico City FIR'},
    'MMMX': {'country': 'Mexico',    'flag': '🇲🇽', 'name': 'Monterrey FIR'},
    'SVZM': {'country': 'Venezuela', 'flag': '🇻🇪', 'name': 'Maiquetia FIR'},
    'SKEC': {'country': 'Colombia',  'flag': '🇨🇴', 'name': 'Bogota FIR'},
    'SPIM': {'country': 'Peru',      'flag': '🇵🇪', 'name': 'Lima FIR'},
    'SBBS': {'country': 'Brazil',    'flag': '🇧🇷', 'name': 'Brasilia FIR'},
    'SUEO': {'country': 'Uruguay',   'flag': '🇺🇾', 'name': 'Montevideo FIR'},
    'SARE': {'country': 'Argentina', 'flag': '🇦🇷', 'name': 'Buenos Aires FIR'},
    'MHCC': {'country': 'Honduras',  'flag': '🇭🇳', 'name': 'Tegucigalpa FIR'},
    'MGGT': {'country': 'Guatemala', 'flag': '🇬🇹', 'name': 'Guatemala FIR'},
    'MPZL': {'country': 'Panama',    'flag': '🇵🇦', 'name': 'Panama FIR'},
    'MUHA': {'country': 'Cuba',      'flag': '🇨🇺', 'name': 'Havana FIR'},
    'TNCF': {'country': 'Haiti',     'flag': '🇭🇹', 'name': 'Port-au-Prince FIR'},
}


def _parse_notam(notam_data, location_info):
    ...
    try:
        ...
        if any(w in notam_text for w in ['AIRSPACE CLOSED', 'AIRSPACE CLO', 'FIR CLOSED']):
            notam_type, icon, color = 'AIRSPACE CLOSURE',   '⛔', 'red'
        elif any(w in notam_text for w in ['RESTRICTED', 'PROHIBITED', 'DANGER AREA']):
            notam_type, icon, color = 'FLIGHT RESTRICTION', '🚫', 'orange'
        elif any(w in notam_text for w in ['MILITARY', 'MIL ACT', 'EXERCISE']):
            notam_type, icon, color = 'MILITARY ACTIVITY',  '⚠️', 'yellow'
        elif any(w in notam_text for w in ['AIRPORT CLOSED', 'AD CLOSED', 'RWY CLOSED']):
            notam_type, icon, color = 'AIRPORT CLOSURE',    '🛑', 'purple'
        elif any(w in notam_text for w in ['NAVAID', 'VOR', 'DME', 'ILS', 'U/S']):
            notam_type, icon, color = 'NAVAID OUTAGE',      '📡', 'blue'
        elif any(w in notam_text for w in ['VOLCANIC', 'ASH', 'HAZARD', 'OBSTRUCTION']):
            notam_type, icon, color = 'HAZARD',             '⚠️', 'gray'
        ...
        eff = _fmt_date(notam_data.get('effectiveStart', ''))
        exp = _fmt_date(notam_data.get('effectiveEnd',   ''))
        ...
        return {
            ...
            'effective_date': eff,
            'expiry_date':    exp,
            'valid_range':    f"{eff} - {exp}" if exp else f"From {eff}",
            'notam_text':     notam_text,
            'source_url':     f"https://notams.aim.faa.gov/notamSearch/notam.html?id={notam_id}"
        }
    except Exception as e:
        print(f'[WHA NOTAM] Parse error: {e}')
        return None


@app.route('/api/notams', methods=['GET', 'OPTIONS'])
def api_wha_notams():
    """Fetch active NOTAMs for WHA region via FAA public API."""
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        for icao_code, info in WHA_FIRS.items():
            try:
                resp = requests.get(
                    base_url,
                    params={'locationICAOId': icao_code, 'responseType': 'application/json'},
                    timeout=5
                )
                if resp.status_code == 200:
                    data = resp.json()
                    for notam in data.get('notamList', []):
                        parsed = _parse_notam(notam, info)
                        if parsed:
                            notams.append(parsed)
                time.sleep(0.2)
            except Exception as e:
                print(f'[WHA NOTAM] Error for {icao_code}: {e}')
                continue
        ...
        print(f'[WHA NOTAM] Fetch error: {e}')
        return jsonify({'notams': [], 'count': 0, 'error': str(e)}), 500


# ========================================
# FLIGHT CANCELLATIONS TRACKER
# ========================================

import xml.etree.ElementTree as ET
import re as _re

_flight_cache_lock = threading.Lock()
_flight_cache_data = {'cancellations': [], 'count': 0, 'last_updated': None}


def _extract_airline(title):
    airlines = [
        'American Airlines', 'United Airlines', 'Delta', 'Southwest',
        'JetBlue', 'Spirit', 'Frontier', 'Alaska Airlines',
        'Air Canada', 'WestJet', 'Aeromexico', 'Volaris', 'VivaAerobus',
        'LATAM', 'Avianca', 'Copa Airlines', 'Wingo',
        'Gol', 'Azul', 'TAM',
        'Caribbean Airlines', 'InterCaribbean',
        'Cubana', 'Air Transat',
    ...
    for airline in airlines:
        if airline.lower() in tl:
            return airline
    m = _re.search(r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(?:suspend|cancel|halt|resume)', title, _re.IGNORECASE)
    if m:
        candidate = m.group(1)
        if len(candidate) > 3 and candidate not in ['United States', 'Latin America']:
            return candidate
    return 'Unknown Airline'


def _extract_status(title):
    tl = title.lower()
    if any(w in tl for w in ['resume', 'restart', 'return']):
        return 'Resumed'
    if 'cancel' in tl:
        return 'Cancelled'
    if any(w in tl for w in ['suspend', 'halt', 'stop']):
        return 'Suspended'
    return 'Disrupted'


def _extract_duration(title):
    m = _re.search(r'until\s+([A-Za-z]+\s+\d{1,2}(?:,?\s+\d{4})?)', title, _re.IGNORECASE)
    if m:
        return f"Until {m.group(1)}"
    for month in ['January', 'February', 'March', 'April', 'May', 'June',
                  'July', 'August', 'September', 'October', 'November', 'December']:
        if month.lower() in title.lower():
            y = _re.search(r'\b(202[4-9])\b', title)
            return f"Until {month} {y.group(1)}" if y else f"Until {month}"
    m2 = _re.search(r'for\s+(\d+)\s+(day|week|month)s?', title, _re.IGNORECASE)
    if m2:
        n, u = m2.group(1), m2.group(2)
        return f"For {n} {u}{'s' if int(n) > 1 else ''}"
...
def _fetch_flight_disruptions():
    """Scan Google News RSS for WHA flight cancellation news."""
    print('[WHA Flights] Starting disruption scan...')

    destinations = [
        'Mexico City', 'Mexico', 'Caracas', 'Venezuela',
        'Bogota', 'Colombia', 'Havana', 'Cuba',
        'Port-au-Prince', 'Haiti', 'Panama City', 'Panama',
        'Sao Paulo', 'Brazil', 'Lima', 'Peru',
    ]
    keywords = ['airline suspended flights', 'flight cancellation']
    ...
    for destination in destinations[:8]:
        for keyword in keywords:
            query = f'{keyword} {destination}'.replace(' ', '+')
            ...
            try:
                ...
                for item in items[:3]:
                    ...
                    all_disruptions.append({
                        'airline':     _extract_airline(title),
                        'route':       f'Various → {destination}',
                        'origin':      'Various',
                        'destination': destination,
                        'date':        _parse_rss_date(d_el.text if d_el is not None else ''),
                        'duration':    _extract_duration(title),
                        'status':      _extract_status(title),
                        'source_url':  link,
                        'headline':    title[:150],
                    })
            except Exception as e:
                print(f'[WHA Flights] Error for {destination}: {str(e)[:80]}')
                continue

    print(f'[WHA Flights] ✅ Found {len(all_disruptions)} disruptions')
    return all_disruptions


def _refresh_flight_cache():
    """Background thread: refresh flight disruption cache every 12 hours."""
    time.sleep(120)  # boot delay — let country scans start first
    while True:
        try:
            disruptions = _fetch_flight_disruptions()
            with _flight_cache_lock:
                _flight_cache_data['cancellations'] = disruptions
                _flight_cache_data['count']         = len(disruptions)
                _flight_cache_data['last_updated']  = datetime.now(timezone.utc).isoformat()
            print('[WHA Flights] Cache refreshed')
        except Exception as e:
            print(f'[WHA Flights] Cache refresh error: {e}')
        time.sleep(12 * 3600)


# Start flight cache refresh thread on boot
threading.Thread(target=_refresh_flight_cache, daemon=True).start()
print('[WHA Backend] Flight disruption refresh thread started')


@app.route('/flight-cancellations', methods=['GET', 'OPTIONS'])
def api_wha_flight_cancellations():
    """Flight disruption data for WHA — served from in-memory cache."""
    if request.method == 'OPTIONS':
        return '', 200
    try:
        with _flight_cache_lock:
            data = dict(_flight_cache_data)
        data['success'] = True
        return jsonify(data)
    except Exception as e:
        print(f'[WHA Flights] Endpoint error: {e}')
        return jsonify({'success': False, 'error': str(e), 'cancellations': [], 'count': 0}), 500
